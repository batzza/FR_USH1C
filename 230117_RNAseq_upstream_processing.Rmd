...
import json
import glob
samples = glob.glob('data/kallisto/*')
...
nm <- dim(meta)[1]
expt.pheno <- factor(paste(meta$Genotype,meta$Age,sep="_"))
...
remove.samp <- c(18) #Remove a sample(s)
...
meta$Group <- paste(meta$Genotype,meta$Age,sep="_")
...
# Import annotation
annot.gene <- read.csv(file = "data/external/Hs_ENS102_GeneAnnot.tsv", sep="\t", row.names = 1)
annot.trans <- read.csv(file = "data/external/Hs_ENS102_TransAnnot.tsv", sep="\t", row.names = 1)
...
write.table(data.frame(merge(gene.dge.filt$genes, gene.dge.filt$lcpm, by = 0), row.names = 1), 
            file=paste0("data/processed/Gene_filtlog2CPM_MSTR_", exprlvl, "CPM.tsv"), 
            quote=F, sep="\t", col.names = NA)
...
```{r}
save.image("data/processed/Processed_RNAseq.Rdata")
```
