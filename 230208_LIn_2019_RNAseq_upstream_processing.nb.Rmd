---
title: "USH1C Organoids -- external data -- Lin 2019 PMID:31652328"
...
import json
import glob
samples = glob.glob('data/external/GSE124532/*')
...
####Get meta file
```{r}
meta <- read.csv("data/external/GSE124532/meta.csv", stringsAsFactors = F)
meta
```
...
nm <- dim(meta)[1]
expt.pheno <- factor(meta$Treatment)
...
```{r}
# Get annotation
# source("data/src/ENSannotDownload_v0.2.0.R")

# Import annotation
annot.gene <- read.csv(file = "data/external/Mm_ENS102_GeneAnnot.tsv", sep="\t", row.names = 1)
annot.trans <- read.csv(file = "data/external/Mm_ENS102_TransAnnot.tsv", sep="\t", row.names = 1)
...
## Sample colors
label.col <- c("#61ADF1","#FFE186","#10518A","#F7B906","#082640", "#f76a06")
...
wd <- "data/external/GSE124532"
samples <- dir(path=wd, pattern="S.+")
...
trans.dge$rpkm[1:6, 1:5]
gene.dge$cpm[1:6, 1:5]
```
...
write.table(trans$counts, file="data/interim/Lin_2019_Trans_Counts_raw.tsv", quote=F, sep="\t", col.names = NA)
write.table(trans$abundance, file="data/interim/Lin_2019_Trans_TPM_raw.tsv", quote=F, sep="\t", col.names = NA)
write.table(trans$length, file="data/interim/Lin_2019_Trans_EffLength_raw.tsv", quote=F, sep="\t", col.names = NA)

write.table(gene$counts, file="data/interim/Lin_2019_Gene_Counts_raw.tsv", quote=F, sep="\t", col.names = NA)
write.table(gene$abundance, file="data/interim/GLin_2019_ene_TPM_raw.tsv", quote=F, sep="\t", col.names = NA)
write.table(gene$length, file="data/interim/Lin_2019_Gene_EffLength_raw.tsv", quote=F, sep="\t", col.names = NA)


#Export transcript tables
write.table(data.frame(merge(trans.dge$genes, trans.dge$rpkm, by=0), row.names=1), 
            file="data/processed/Lin_2019_Trans_RPKM_MSTR.tsv", quote=F, sep="\t", col.names = NA)
write.table(data.frame(merge(trans.dge$genes, trans.dge$lrpkm, by=0), row.names=1), 
            file="data/processed/Lin_2019_Trans_log2RPKM_MSTR.tsv", quote=F, sep="\t", col.names = NA)


#Export gene tables
write.table(data.frame(merge(annot.gene, gene.dge$cpm, by=0), row.names=1), 
            file="data/processed/Lin_2019_Gene_CPM_MSTR.tsv", quote=F, sep="\t", col.names = NA)
write.table(data.frame(merge(annot.gene, gene.dge$lcpm, by=0), row.names=1), 
            file="data/processed/Lin_2019_Gene_log2CPM_MSTR.tsv", quote=F, sep="\t", col.names = NA)
...
```{r}
source("data/src/filterFPKM_v1.R")
...
write.table(data.frame(merge(gene.dge.filt$genes, gene.dge.filt$cpm, by = 0), row.names = 1), 
            file=paste0("data/processed/Lin_2019_Gene_filtCPM_MSTR_", exprlvl, "CPM.tsv"), 
...
write.table(data.frame(merge(gene.dge.filt$genes, gene.dge.filt$lcpm, by = 0), row.names = 1), 
            file=paste0("data/processed/Lin_2019_Gene_filtlog2CPM_MSTR_", exprlvl, "CPM.tsv"), 
            quote=F, sep="\t", col.names = NA)
...
```{r}
save.image("data/processed/Lin_2019_Processed_RNAseq.Rdata")
```
